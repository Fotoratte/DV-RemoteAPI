...
namespace RemoteAPI.Api
{
    public class ApiServer
    {
        ...
        private static string GetPrefix(string host, int port) => $"http://{host}:{port}/";
        ...
        public void Start()
        {
            try
            {
                listener.Prefixes.Add(GetPrefix(settings.Host, settings.Port));
                listener.Start();
        ...
        {
            var prefix = GetPrefix(settings.Host, settings.Port);
